...
const BuildConfig = struct {
    dependency_roots: []DepsBuildRoots,
    packages: []Pkg,
...
pub fn main() !void {
    ...
    var include_directories: std.StringArrayHashMapUnmanaged(void) = .{};
    ...
    for (builder.top_level_steps.values()) |tls| {
        for (tls.step.dependencies.items) |step| {
            try processStep(builder, &packages, &include_directories, step);
        }
    }

    var dependency_roots: std.ArrayListUnmanaged(BuildConfig.DepsBuildRoots) = .{};
    for (dependencies.root_deps) |root_dep| {
        inline for (@typeInfo(dependencies.packages).Struct.decls) |package| blk: {
            if (std.mem.eql(u8, package.name, root_dep[1])) {
                ...
                try dependency_roots.append(allocator, .{
                    .name = root_dep[0],
    ...
    try std.json.stringify(
        BuildConfig{
            .dependency_roots = try dependency_roots.toOwnedSlice(allocator),
            .packages = try packages.toPackageList(),
            .include_directories = include_directories.keys(),
        },
        .{},
...
fn processStep(
    builder: *std.Build,
    packages: *Packages,
    include_directories: *std.StringArrayHashMapUnmanaged(void),
    step: *Build.Step,
) anyerror!void {
    for (step.dependencies.items) |dependant_step| {
        try processStep(builder, packages, include_directories, dependant_step);
    }
    ...
    try processPkgConfig(builder.allocator, include_directories, exe);
    ...
    try processModule(builder, packages, include_directories, exe.root_module);
}
...
fn processModule(
    ...
    try processIncludeDirs(builder, include_directories, module.include_dirs.items);
    ...
    for (module.import_table.keys(), module.import_table.values()) |name, mod| {
        ...
        try processModule(builder, packages, include_directories, mod.*);
    }
}
    ...
    for (dirs) |dir| {
        switch (dir) {
            .path, .path_system, .path_after => |path| {
                ...
                try include_directories.put(builder.allocator, resolved_path, {});
            },
            .other_step => |other_step| {
                if (other_step.generated_h) |header| {
                    if (header.path) |path| {
                        try include_directories.put(builder.allocator, std.fs.path.dirname(path).?, {});
                    }
                }
                if (other_step.installed_headers.items.len > 0) {
                    ...
                    try include_directories.put(builder.allocator, path, {});
                }
            },
            .config_header_step => |config_header| {
                const full_file_path = config_header.output_file.path orelse continue;
                const header_dir_path = full_file_path[0 .. full_file_path.len - config_header.include_path.len];
                try include_directories.put(builder.allocator, header_dir_path, {});
            },
...
fn processPkgConfig(
    ...
    for (exe.root_module.link_objects.items) |link_object| {
        ...
        getPkgConfigIncludes(allocator, include_directories, exe, system_lib.name) catch |err| switch (err) {
    ...
    if (copied_from_zig.runPkgConfig(exe, name)) |args| {
        for (args) |arg| {
            if (std.mem.startsWith(u8, arg, "-I")) {
                ...
                try include_directories.put(allocator, candidate, {});
            }
        }
    } else |err| return err;
}
